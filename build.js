...
function fromEvent$1(element, eventName, useCapture, preventDefault) {
    if (useCapture === void 0) { useCapture = false; }
    if (preventDefault === void 0) { preventDefault = false; }
    return xstream_3.create({
        element: element,
        next: null,
...
var DocumentDOMSource = (function () {
    ...
    DocumentDOMSource.prototype.events = function (eventType, options) {
        ...
        var stream;
        stream = fromEvent$1(document, eventType, options.useCapture, options.preventDefault);
...
var BodyDOMSource = (function () {
    ...
    BodyDOMSource.prototype.events = function (eventType, options) {
        if (options === void 0) { options = {}; }
        var stream;
        stream = fromEvent$1(document.body, eventType, options.useCapture, options.preventDefault);
        var out = adapt_3(stream);
        out._isCycleSource = this._name;
        return out;
    };
...
var MainDOMSource = (function () {
    ...
    MainDOMSource.prototype.elements = function () {
        ...
        var out = adapt_3(output$.remember());
        out._isCycleSource = this._name;
        return out;
    };
    ...
    MainDOMSource.prototype.events = function (eventType, options) {
        ...
        var event$ = rootElement$
            ...
            if (!namespace || namespace.length === 0) {
                return fromEvent$1(rootElement, eventType, useCapture, options.preventDefault);
            }
...
var thunk$2 = createCommonjsModule(function (module, exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
var VNodeWrapper = (function () {
    ...
    VNodeWrapper.prototype.call = function (vnode$$1) {
        ...
        var _a = selectorParser_2(vnode$$1), selTagName = _a.tagName, selId = _a.id;
        var vNodeClassName = classNameFromVNode_2(vnode$$1);
...
var StyleModule = unwrapExports(style$1);
...
var isObject = function (value) {
	return (isValue(value) && map$1[typeof value]) || false;
};
...
var autoBind = function (props/*, options*/) {
	...
	return map$2(props, function (desc, name) { return define(name, desc, options); });
};
...
function makeDOMDriver(container, options) {
    ...
    function DOMDriver(vnode$, name) {
        ...
        var rootElement$ = xs
            ...
            .fold(patch, tovnode_1(rootElement))
            .drop(1)
...
var MockedDOMSource = (function () {
    function MockedDOMSource(_mockConfig) {
        ...
        if (_mockConfig['elements']) {
            ...
            this._elements = adapt_3(xs.empty());
        }
    }
    ...
    MockedDOMSource.prototype.events = function (eventType, options) {
        ...
        var out = adapt_3(streamForEventType || xs.empty());
        out._isCycleSource = 'MockedDOM';
        return out;
    };
    ...
    MockedDOMSource.prototype.isolateSource = function (source, scope) {
        return source.select('.' + SCOPE_PREFIX$1 + scope);
    };
    MockedDOMSource.prototype.isolateSink = function (sink, scope) {
        return sink.map(function (vnode) {
            if (vnode.sel && vnode.sel.indexOf(SCOPE_PREFIX$1 + scope) !== -1) {
                return vnode;
            }
            else {
                vnode.sel += "." + SCOPE_PREFIX$1 + scope;
                return vnode;
            }
        });
    };
...
var SVG_TAG_NAMES = [
    ...
    'view',
    'vkern' ];
var svg$1 = createTagFunction('svg');
SVG_TAG_NAMES.forEach(function (tag) {
    svg$1[tag] = createTagFunction(tag);
});
var TAG_NAMES = [
    'a',
    'abbr',
...
var gridView = function (grid, width, height) { return h_2('g.grid', lodash.flatMap(grid, function (vertex, direction) {
            if (direction === 'x') {
                return vertex.map(function (n) { return h_2('line.x', {
                        attrs: {
                            x1: n,
                            x2: n,
                            y1: 0,
            ...
            if (direction === 'y') {
                return vertex.map(function (n) { return h_2('line.y', {
                        attrs: {
                            x1: 0,
                            x2: width,
                            y1: n,
...
var triangleView = function (triangle, verticesRadius) { return h_2('g.triangle', triangle.map(function (vertex, i$$1, triangle) {
            var vertex2 = triangle[i$$1 + 1] || triangle [0];
            return h_2('line', {
                attrs: {
                    x1: vertex.x,
                    y1: vertex.y,
        ...
        }).concat( triangle.map(function (vertex, index) { return h_2('g', [
                h_2('circle.vertex', {
                    attrs: {
                        cx: vertex.x,
                        cy: vertex.y,
...
var pointsView = function (points, radius) { return h_2('g.points', points.map(function (point) { return h_2('circle.dot', {
            attrs: {
                cx: point.x,
                cy: point.y,
                r: radius
...
function makeRandomAdimTriangle(withHorizontalSide) {
    return lodash.chain(3)
        .range()
        .map(function (i$$1) { return ({x: Math.random(), y: Math.random()}); } )
        .tap(function (triangle) {
        ...
        .sortBy(function (vertex) { return vertex.y; })
        .value();
}

var lineXAtY = function (ref) {
    var p1 = ref[0];
    var p2 = ref[1];
...
var makeSnapBoundary = function (spacing) { return function (n, i$$1) {
        var mod = n % spacing;
        return (mod === 0)
            ? n + (i$$1 === 1 ? spacing : 0)    // range excludes last element
            : n - mod + spacing
    }; };
...
function main(ref) {
    ...
    var click$ = DOM.select('svg').events('click');

    var adimTriangle$ =
        click$
        .fold(function (clicks) { return clicks + 1; }, 0)
        .map(function (n) { return makeRandomAdimTriangle(n % 4 === 0); });
    ...
    var state$ =
        ...
        .map(function (ref) {
            var area$$1 = ref[0];
            var adimTriangle = ref[1];

            var width = area$$1.width;
            var height = area$$1.height;
            var padding = area$$1.padding;
            var spacing = area$$1.spacing;

            var triangle = adimTriangle.map(function (v) { return ({
                x: padding + (width - 2 * padding) * v.x,
                y: padding + (height - 2 * padding) * v.y
            }); });

            var s01 = [triangle[0], triangle[1]];
            var s02 = [triangle[0], triangle[2]];
            var s12 = [triangle[1], triangle[2]];

            var snapBoundary = makeSnapBoundary(spacing);

            var extentAtYInSides = function (sides) { return function (y) { return lodash.chain(sides)
                    .map(function (side) { return lineXAtY(side)(y); })
                    .sort(function (a$$1, b$$1) { return a$$1 - b$$1; })
                    .map(snapBoundary)
                    .value(); }; };

            var extentAtY01 = extentAtYInSides([s01, s02]);
            var extentAtY12 = extentAtYInSides([s02, s12]);

            var pointsInRange01 = (triangle[0].y === triangle[1].y)
                ? (triangle[0].y % spacing === 0)
                    ? lodash.map(
                        lodash.range.apply(lodash, [triangle[0].x, triangle[1].x].map(snapBoundary).concat( [spacing] )),
                        function (x) { return ({ x: x, y: triangle[0].y }); }
                    ) : []
                : lodash.flatMap(
                    lodash.range.apply(
                        lodash, [triangle[0].y, triangle[1].y].map(snapBoundary).concat( [spacing] )
                    ), function (y) { return lodash.range.apply(lodash, extentAtY01(y).concat( [spacing] ))
                        .map(function (x) { return ({x: x, y: y}); }); }
                );
            var pointsInRange12 = (triangle[1].y === triangle[2].y)
                ? (triangle[1].y % spacing === 0)
                    ? lodash.map(
                        lodash.range.apply(lodash, [triangle[1].x, triangle[2].x].map(snapBoundary).concat( [spacing] )),
                        function (x) { return ({ x: x, y: triangle[1].y }); }
                    ) : []
                : lodash.flatMap(
                    lodash.range.apply(
                        lodash, [triangle[1].y, triangle[2].y].map(snapBoundary).concat( [spacing] )
                    ), function (y) { return lodash.range.apply(lodash, extentAtY12(y).concat( [spacing] ))
                        .map(function (x) { return ({x: x, y: y}); }); }
                );

            var points = pointsInRange01.concat( pointsInRange12);

            return {
                area: area$$1,
                triangle: triangle,
                grid: {
                    x: lodash.range(spacing, width, spacing),
                    y: lodash.range(spacing, height, spacing),
                },
                points: points
            }
        });
    ...
    var vdom$ = state$.map(function (state) { return h_2('svg', {
            attrs: {
